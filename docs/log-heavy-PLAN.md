...
## 7. 実測結果（2026-09-07・実装後の追記）

実装ブランチ `ebi/log-heavy-impl`。案 1（rAF ＋件数上限）と案 2（`cleared`）を入れた。案 3 は着手せず。

計測条件は §3 と同じ（`tmp/log-heavy/` のハーネス・Vite 5175・Playwright Chromium 1280x900）。
実ログが伸びたので投入イベントは 22,710 → **22,949 件**（＝ 3,589 アイテム）になっている。

計測にあたりハーネス側も 2 点直した（before/after は同じハーネスで取り直している）:

- `__streamFrame` の seq が `__stream` より小さく、重複配信として捨てられていた
- `__measure` がレイアウト確定を待たずに終わっていた（rAF 化するとコストがフレーム側へ移るため、
  待たないと「速くなった」ように見えてしまう）

### 22,949 イベント（＝ボスの 2 日ぶん）での before / after

| 指標 | before | ① rAF 化のみ | ② ＋件数上限 400 |
|---|---:|---:|---:|
| アイテム数 | 3,589 | 3,589 | **400** |
| `.chat-log` 配下 DOM ノード | 31,764 | 31,764 | **3,708** |
| scrollHeight | 566,181px | 566,181px | **60,380px** |
| 初期描画（render＋layout） | 1,243ms | 1,249ms | **216ms** |
| **streaming 1 トークン** | **24.97ms** | **0.05ms** | **0.07ms** |
| 同（次フレーム確定まで込み） | 26.26ms | 0.26ms | 0.11ms |

### 投入量別（実装後）

| 投入イベント数 | アイテム数 | DOM ノード | scrollHeight | 初期描画(render+layout) | streaming 1 トークン |
|---:|---:|---:|---:|---:|---:|
| 400 | 48 | 314 | 5,618px | 19.6＋35.4ms | 0.08ms |
| 1,000 | 158 | 1,209 | 21,253px | 26.6＋86.3ms | 0.07ms |
| 4,000 | **400**（上限） | 3,708 | 60,380px | 37.8＋169.4ms | 0.06ms |
| 22,949 | **400**（上限） | 3,708 | 60,380px | 42.0＋174.1ms | 0.07ms |

読み方:

- **主因（同期強制リフロー）は rAF 化だけで消える**。24.97ms → 0.05ms（約 1/500）。ここが最も費用対効果が高い、という §5 の見立てはそのとおりだった。
- rAF 化は初期描画には効かない（レイアウト自体は必要なので、コストがフレーム側へ移るだけ・1,243ms → 1,249ms）。**初期描画を落としたのは件数上限のほう**（216ms）。両方入れて意味がある。
- 上限を掛けたあとは、どれだけイベントを流し込んでも DOM・scrollHeight・描画コストが一定になる（4,000 と 22,949 が同値）。**開きっぱなしでも重くならない**が達成できている。

### `cleared`（案 2）の DOM 実測

22,949 件を流し込んだ状態（400 アイテム）で `cleared` → 発話 1 件を適用したあと:

| | cleared 直前 | cleared 直後 |
|---|---:|---:|
| アイテム数 | 400 | 2 |
| `.chat-log` 配下 DOM ノード | 3,708 | 8 |
| scrollHeight | 60,380px | 789px |

行の並びは `（これより前の会話はログファイルにのみ残っています）` → 区切り行 → リセット後の発話。

### テスト

- `npm test` … 500 tests / 499 pass / 0 fail / 1 skipped
- `npx tsc --noEmit`（`npm run typecheck` の client 側）… エラー無し
- `EBI_E2E_CHAT_ROUNDS=1 EBI_E2E_CHAT_PORT=8799 node scripts/e2e-master-chat.mjs` … 10/10 OK
  （`chatNew → cleared` が live と再接続 snapshot の両方に載る／JSONL は切り詰められない、を追加）
