...
- **役割プロンプトへの ACK は監視する**（§4.1）。ACK に「reply_to_master ツールが利用できない」
  と書いて以後タスクを実行しない**静かな故障**が一定確率で起きるため（§7.1）、その文面を
  検知して 1 回だけエビを作り直す。
- 起動ゲート/更新ダイアログ/ログイン要求は `fatalPatterns` で検知し、notice とサーバログに
  原因を 1 行で残す（黙って ready 待ちタイムアウトさせない）。

### 4.1 ACK 監視と「静かな故障」の作り直し

役割プロンプトを注入したあと、**その ACK（最初の応答）だけを監視する**。

| 何を | どこで | 値 |
|---|---|---|
| 検知パターン | `CODEX_ACK_FAILURE_PATTERNS`（`backends/profiles.ts`） | `(利用\|使用)でき(ない\|ません\|ず)` / `(呼び出せません\|呼び出せない\|使えません)` / `(提供\|登録\|用意)されて(いない\|いません\|おらず)` / `(実行\|送信\|報告)でき(ない\|ません\|ず)` / 英語の `tool … not available/unavailable/not provided` |
| 監視窓 | `CODEX_ACK_FAILURE_WATCH.windowMs` | 既定 90 秒（env `EBI_CODEX_ACK_WATCH_MS`） |
| 早すぎる終了の防止 | `.minObserveMs` | 既定 3 秒（env `EBI_CODEX_ACK_MIN_OBSERVE_MS`） |
| 走査 | `agent.ts` の `maybeDetectAckFailure` | ANSI 除去後の素文・直近 8KB のリングバッファ |
| 判断 | `ackRespawn.ts` の `decideAckFailureAction`（純関数） | 1 回目=作り直し / 2 回目=fatal / 対象外=無視 |
| 実行 | `index.ts` の `handleAckFailure` | kill（プロセスグループ）→ 同一 id・同一引数で再 spawn |

監視の開始と終了:

- **開始**: 役割プロンプトの初回注入（`sendInitialInject`）と同時。
- **終了（成功）**: ACK が届いた（busy→idle のエッジ）時点で、パターンに一致していなければ終了。
  注入本文のエコーだけで busy→idle が一往復しうるため、`minObserveMs` を過ぎるまでは
  「ACK 到着」とみなさない。
- **終了（上限）**: `windowMs` 経過。ACK が来ないまま黙るケースを永久に疑わない。
- **終了（検知）**: パターン一致。以後このエビでは二度と発火しない。

検知後の動作:

1. **1 回目**: 監視中に届いた注入本文（master が送ったタスク）を取り出す → `registry.remove()` で
   kill（`killProcessGroup` によりプロセスグループごと）→ **旧プロセスの exit を待ってから**
   同一 id・同一引数で再 spawn → ready 後に滞留していた本文を新エビへ引き継ぐ。
   引き継がないと「エビは作り直したがタスクは消えた」となり、直そうとしている故障そのものになる。
   引き継ぐのは**注入キューではなく「ACK 監視中に届いた本文」**（`takeAckWindowBodies`）である点が肝:
   ready 直後のエビは idle なので、master のタスクはキューを経ずに**故障エビの PTY へ直接書かれる**。
   キューだけを見ていると 1 件静かに消える（実測: 実装直後の e2e round 2）。
   exit を待つのは、exit ハンドラが id だけを見て registry から remove するため（待たずに立てると
   遅れて届いた旧 exit が**新エビを kill する**）。
2. **2 回目**（作り直したエビが再び同じ ACK を返した場合）: もう作り直さず、
   **fatal として master へ通知**する（`reverseInject`＝既存の通知経路）。本文に「タスクは未実行」
   「別 backend（claude）で振り直してください」を含める。黙って idle のまま放置しない。

ログ（`EBI_DELIVERY_LOG_PATH` の JSONL）に残る event:
`ack-failure-respawn`（1 回目）/ `ack-failure-fatal`（2 回目）/ `ack-failure-requeued`
（滞留注入の引き継ぎ成功）/ `ack-failure-requeue-failed`。UI へは notice で流れる。

**役割プロンプト本文に検知語を入れないこと。** 注入本文は TUI がそのままエコーするため、
役割プロンプトに「利用できません」等を書くと自分のエコーで誤検知する
（`test/codexAckRespawn.test.ts` が組込み役割・e2e のタスク本文で錠前を掛けている）。

claude / gemini は `ackFailureWatch` 未設定＝**挙動不変**（監視も作り直しもしない）。
...
| `EBI_CODEX_ACK_WATCH_MS` | `90000` | 役割プロンプト ACK の監視上限（§4.1） |
| `EBI_CODEX_ACK_MIN_OBSERVE_MS` | `3000` | ACK 到着とみなすまでの最小観測時間（§4.1） |
| `EBI_ACK_RESPAWN_EXIT_WAIT_MS` | `8000` | 作り直し時に旧プロセスの exit を待つ上限 |
...
1. **ACK 文面での早期検出＋再 spawn** — **実装済み**（§4.1・成績は §7.2）。失敗は役割プロンプトへの
   応答時点で確定しており、`利用でき(ない|ません)` を含む応答を検知して 1 回だけ作り直す。
   各ラウンド独立と仮定した単純計算では失敗率 13.3% → 約 1.8%
2. ready 直後に `/mcp` を注入してサーバ側でツール列挙を確認する案 — **実装しない**（比較は §7.3）
3. 報告経路を PTY 依存にしない（`<worktree>/.ebi-report.md` の watch・設計書 R3）。1 で足りるなら不要
...
- 上の「次の一手」1（ACK 検出＋再 spawn）を入れて **10/10 を 2 セット連続**で取れたら、
  ...
  → 実装後の 1 セット目は **10/10**（§7.2）。ただし静かな故障そのものは 4 ラウンドで発生しており
  「観測ゼロ」ではない（作り直しで回復しているだけ）。**昇格の判断はこの節では行わない**（数字のみ）
...
### 7.2 実装後の e2e 成績（2026-09-05・ACK 検知＋作り直しを入れた状態・10 ラウンド）

**10/10（100%）。うち 4 ラウンドで「静かな故障」を検知し、作り直しで回復した。**
条件: `codex-cli 0.146.0` / `gpt-5.6-terra` / 無料 ChatGPT 垢 / Darwin 25.4.0 /
作りたて dist（`npm ci` → `npm run build`）/ 専用ポート 8823 / 稼働サーバ（8787）非依存。

| round | ok | reply 所要 | 検知 | 作り直し | 引き継ぎ | 備考 |
|---|---|---|---|---|---|---|
| 1 | ✅ | 29.5s | — | — | — | |
| 2 | ✅ | 31.0s | — | — | — | |
| 3 | ✅ | 57.0s | 1 | 1 | 1 件 | 作り直しで回復 |
| 4 | ✅ | 29.0s | — | — | — | |
| 5 | ✅ | 53.0s | 1 | 1 | 1 件 | 作り直しで回復 |
| 6 | ✅ | 30.9s | — | — | — | |
| 7 | ✅ | 57.0s | **2** | 1 | 1 件 | 2 回目も検知 → master へ fatal 通知。**ただしその後タスクは完遂**（下記） |
| 8 | ✅ | 29.0s | — | — | — | |
| 9 | ✅ | 35.0s | — | — | — | |
| 10 | ✅ | 55.0s | 1 | 1 | 1 件 | 作り直しで回復 |
| 合計 | **10/10** | — | **5 回** | **4 回** | 4 件 | fatal 通知 1 回 |

- **検知が発火した回数: 5**（ラウンド 3 / 5 / 7×2 / 10）。**再 spawn: 4 回**、**fatal 通知: 1 回**。
- 素の ACK 失敗率は 4/10（作り直し前の 1 回目だけを数えた場合）。§7.1 の 2/15 より高いが、
  これは日・セッションによるばらつきの範囲（母数が小さい）。**重要なのは、失敗しても
  ラウンドが落ちなくなったこと**（作り直し前は同じ失敗がそのままラウンド失敗だった）。
- 回復ラウンドの reply 所要は 53〜57 秒（通常 29〜35 秒）。作り直しのコストは **+約 25 秒**で、
  e2e のラウンド上限 180 秒には十分収まる。
- 全 10 ラウンドで idle 復帰 OK・`pgrep -x codex` = 0・制御MCP の残存 0。

#### 2 回目の検知（fatal 通知）は「誤報」でもありうる

round 7 は、作り直したエビが再び ACK で

> 了解しました。reply_to_master ツールはこの環境では利用できないため、現時点では master へ送信できません。

と述べたため fatal 通知を出した（仕様どおり 3 回目は作らない）。**しかしその直後、引き継いだ
タスク本文を受け取ったエビは `reply_to_master({"message":"PONG7XW7ZF"})` を実際に呼んで成功した。**

つまり **ACK の「使えません」は必ずしも終局ではない**。§7.1 の観測（「一度そう述べたセッションは
回復しない」）は 2 ラウンドの標本に基づくもので、今回それに当てはまらない例が出た。
運用上の含意:

- 1 回目の検知で作り直す方針は変えなくてよい（コストは +25 秒で、取り返せる損失の方が大きい）
- **2 回目の fatal 通知は「タスクが失われた」の確定通知ではなく「怪しいので人間/master が見ろ」**
  として読むこと。実際に報告が届けば、master は fatal 通知と正しい報告の両方を受け取る
- 逆に言えば、fatal 通知を受けた master が即座にタスクを別エビへ振り直すと**二重実行**になりうる。
  振り直す前に read_scrollback で確認するのが安全

#### 実装中に潰した 2 つの穴（いずれも live e2e で発覚）

この 10/10 に至る前に、途中で打ち切った測定が 2 本ある（どちらも意図的に中断し、
codex 残存 0 を確認済み）。

1. **検知パターンの取りこぼし**（1 本目・round 2 で発覚）。作り直したエビが
   「この環境には reply_to_master ツールが**提供されていない**ため、master へのツール経由の報告は
   **実行できません**」と別の言い回しで失敗し、`利用でき` 系だけの初版では検知できなかった。
   → `(提供|登録|用意)されて(いない|…)` と `(実行|送信|報告)でき(ない|…)` を追加（§4.1）
2. **引き継ぎが空だった**（2 本目・round 2 で発覚）。作り直しは成功したのにタスクが届かず、
   ラウンドは timeout。原因は「ready 直後のエビは idle なので、master のタスクは**注入キューを
   経ずに PTY へ直接書かれる**」こと。注入キューだけを持ち越しても中身は空だった。
   → 「ACK 監視中に届いた本文」を控えて持ち越す方式に変更（`takeAckWindowBodies`・§4.1）

再現手順:

```bash
npm ci && npm run build
env -u EBI_ID EBI_E2E_PORT=8823 EBI_E2E_ROUNDS=10 EBI_E2E_DUMP=1 npm run e2e:codex
```

検知・作り直しの発火はサーバログ（`[delivery:ack-failure-*]`）と配送ログ JSONL で数えられる:

```bash
grep -c '静かな故障を検知' <サーバログ>            # 検知回数（notice と console の 2 行 × 回数）
grep -c 'ack-failure-respawn' <配送ログ>          # 作り直し回数
grep -c 'ack-failure-fatal'  <配送ログ>           # fatal 通知回数
```

---

### 7.3 代替案の比較メモ（実装しない: ready 直後に `/mcp` を注入して確認する）

§7.1「次の一手」2 の案。**比較のみで実装しない**（今回の実装は 1 の ACK 文面検知）。

| 観点 | 実装した案（ACK 文面検知） | 代替案（`/mcp` 注入＋列挙確認） |
|---|---|---|
| 何を見るか | 役割プロンプトへの**応答文** | TUI の `/mcp` 出力に `ebi-control` の 3 ツールが並ぶか |
| 依存 | **モデルの言い回し**（正規表現の網羅性） | codex TUI の `/mcp` 出力書式（CLI バージョン差） |
| 見逃し | 「利用できない」と言わずに黙って報告しないケースは拾えない | ツールが**列挙されるのに使わない**ケースは拾えない |
| 誤検知 | 注入本文のエコー・タスク本文に検知語が入ると誤爆（§4.1 の注意） | 出力書式が変わると常に「ツール無し」と誤判定しうる |
| 追加コスト | ゼロ（既に流れている PTY 出力を走査するだけ） | **ready ごとに 1 往復増える**（`/mcp` 注入 → 描画待ち → 画面パース）。ready→初回タスクの遅延も伸びる |
| 状態の汚染 | なし | エビの画面に `/mcp` の出力が残る（scrollback を読む master 側のノイズ） |
| §7.1 の実測との整合 | 失敗ラウンドでも**制御MCP は立っており**、`mcp=3` も出ていた。つまり「ツールは在る」 | 在るものを数えるので、**今回の失敗の型は検出できない可能性が高い** |

決め手は最後の行。§7.1 の再測定で「MCP プロセス・注入順序・所要時間には差が無く、
モデルの初回応答だけが分かれる」ことが分かっている以上、サーバ側でツールを数えても
**失敗ラウンドと成功ラウンドを区別できない**（どちらも 3 ツール見えるはず）。
`/mcp` 案が効くのは「MCP が本当に起動していない」型の故障で、それは既に `fatalPatterns` と
preflight の担当範囲。

将来 ACK 文面が変わって検知率が落ちたら、次に検討すべきは 2 ではなく §7.1 の 3
（報告経路を PTY 依存にしない）だと考える。
